 ...
 * `.test-build/` first. `@omadia/channel-teams` is kept external so tests
 * exercise the built `dist/` (self-reference via package.json `exports`,
 * requires `npm run build` first); the `@omadia/*` peers are kept external
 * too since tests only import types from them, which are erased. Then
 * `node --test` runs the transpiled output.
 */
...
await build({
  ...
  logLevel: 'error',
  external: [
    // Types-only peer — erased at transpile time, kept external defensively.
    '@omadia/plugin-api',
  ],
});
